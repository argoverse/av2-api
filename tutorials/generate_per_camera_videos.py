...
def generate_per_camera_videos(data_root: Path, output_dir: Path, num_workers: int) -> None:
    ...
    if num_workers > 1:
        Parallel(n_jobs=num_workers)(
            delayed(render_log_ring_camera_videos)(output_dir, loader, log_id) for log_id in log_ids
        )
    else:
...
def render_log_ring_camera_videos(output_dir: Path, loader: AV2SensorDataLoader, log_id: str) -> None:
    ...
    for camera_name in RingCameras:
        video_save_fpath = Path(output_dir) / f"{log_id}_{camera_name}.mp4"
        if video_save_fpath.exists():
            logger.info("Video already exists for %s, %s, so skipping...", log_id, camera_name)
            continue
        ...
        for i, im_fpath in enumerate(cam_im_fpaths):
            if i % PRINT_EVERY == 0:
                logger.info(f"\tOn file {i}/{num_cam_imgs} of camera {camera_name} of {log_id}")

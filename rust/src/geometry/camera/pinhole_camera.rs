...
use crate::{
    frame_utils::{extract_f64_from_frame, extract_usize_from_frame},
    geometry::se3::SE3,
    geometry::so3::_quat_to_mat3,
    geometry::utils::cart_to_hom,
...
impl Intrinsics {
    ...
    pub fn k(&self) -> Array<f64, Ix2> {
        let mut k = Array::<f64, Ix2>::eye(3);
        k[[0, 0]] = self.fx_px;
        k[[1, 1]] = self.fy_px;
        k[[0, 2]] = self.cx_px;
        k[[1, 2]] = self.cy_px;
...
impl Intrinsics {
    ...
    #[pyo3(name = "k")]
    pub fn py_k<'py>(&self, py: Python<'py>) -> &'py PyArray<f64, Ix2> {
...
impl PinholeCamera {
    ...
    pub fn extrinsics(&self) -> Array<f64, Ix2> {
        self.ego_se3_cam.inverse().transform_matrix()
    }

    /// Camera projection matrix.
    pub fn p(&self) -> Array<f64, Ix2> {
        let mut p = Array::<f64, Ix2>::zeros([4, 4]);
        p.slice_mut(s![..3, ..3]).assign(&self.intrinsics.k());
        p
    }

    /// Model matrix.
    pub fn m(&self) -> Array<f64, Ix2> {
        self.extrinsics()
    }
    ...
    pub fn from_feather(log_dir: &Path, camera_name: &str) -> PinholeCamera {
        ...
        let intrinsics = Intrinsics {
            fx_px: extract_f64_from_frame(&intrinsics, "fx_px"),
            fy_px: extract_f64_from_frame(&intrinsics, "fy_px"),
            cx_px: extract_f64_from_frame(&intrinsics, "cx_px"),
            cy_px: extract_f64_from_frame(&intrinsics, "cy_px"),
    ...
    pub fn cull_to_view_frustum(
        self,
        uv: &ArrayView<f64, Ix2>,
        points_camera: &ArrayView<f64, Ix2>,
    ) -> Array<bool, Ix2> {
        ...
        par_azip!((mut is_within_frustum_i in is_within_frustum.outer_iter_mut(), uv_row in uv.outer_iter(), point_cam in points_camera.outer_iter()) {
            let is_within_frustum_x = (uv_row[0] >= 0.) && (uv_row[0] < self.width_px() as f64);
            let is_within_frustum_y = (uv_row[1] >= 0.) && (uv_row[1] < self.height_px() as f64);
    ...
    pub fn project_ego_to_image(
        &self,
        points_ego: ArrayView<f64, Ix2>,
    ) -> (Array<f64, Ix2>, Array<f64, Ix2>, Array<bool, Ix2>) {
...
impl PinholeCamera {
    ...
    pub fn py_extrinsics<'py>(&self, py: Python<'py>) -> &'py PyArray<f64, Ix2> {
        self.extrinsics().into_pyarray(py)
    }

    /// Camera projection matrix.
    #[pyo3(name = "p")]
    pub fn py_p<'py>(&self, py: Python<'py>) -> &'py PyArray<f64, Ix2> {
        self.p().into_pyarray(py)
    }

    /// Camera projection matrix.
    #[pyo3(name = "mvp")]
    pub fn py_mvp<'py>(&self, py: Python<'py>) -> &'py PyArray<f64, Ix2> {
    ...
    pub fn py_project_ego_to_image<'py>(
        &self,
        py: Python<'py>,
        points_ego: PyReadonlyArray<f64, Ix2>,
    ) -> (
        &'py PyArray<f64, Ix2>,
        &'py PyArray<f64, Ix2>,
        &'py PyArray<bool, Ix2>,
    ...
    pub fn py_project_ego_to_image_motion_compensated<'py>(
        ...
        let (uvz, points_hom_cam, is_valid_points) = self.project_ego_to_image_motion_compensated(
            points_ego.as_array(),

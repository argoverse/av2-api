...
pub fn quat_to_mat3(quat_wxyz: &ArrayView<f64, Ix2>) -> Array<f64, Ix3> {
    let num_quats = quat_wxyz.shape()[0];
    let mut mat3 = Array::<f64, Ix3>::zeros((num_quats, 3, 3));
...
/// Convert a quaternion in scalar-first format to a 3x3 rotation matrix.
pub fn _quat_to_mat3(quat_wxyz: &ArrayView<f64, Ix1>) -> Array<f64, Ix2> {
    let w = quat_wxyz[0];
    let x = quat_wxyz[1];
    let y = quat_wxyz[2];
    let z = quat_wxyz[3];
    ...
    let e_01 = 2. * x * y - 2. * z * w;
    let e_02 = 2. * x * z + 2. * y * w;
...
/// Parallelized for batch processing.
pub fn mat3_to_quat(mat3: &ArrayView<f64, Ix3>) -> Array<f64, Ix2> {
    let num_transformations = mat3.shape()[0];
    let mut quat_wxyz = Array::<f64, Ix2>::zeros((num_transformations, 4));
...
/// Convert a 3x3 rotation matrix to a scalar-first quaternion.
pub fn _mat3_to_quat(mat3: &ArrayView<f64, Ix2>) -> Array<f64, Ix1> {
    ...
    let mut quat_wxyz = if trace > 0.0 {
        ...
        Array::<f64, Ix1>::from_vec(vec![qw, qx, qy, qz])
    } else if mat3[[0, 0]] > mat3[[1, 1]] && mat3[[0, 0]] > mat3[[2, 2]] {
        let s = 2.0 * f64::sqrt(1.0 + mat3[[0, 0]] - mat3[[1, 1]] - mat3[[2, 2]]);
        ...
        let s = 2.0 * f64::sqrt(1.0 + mat3[[1, 1]] - mat3[[0, 0]] - mat3[[2, 2]]);
        ...
        let s = 2.0 * f64::sqrt(1.0 + mat3[[2, 2]] - mat3[[0, 0]] - mat3[[1, 1]]);
...
/// Parallelized for batch processing.
pub fn quat_to_yaw(quat_wxyz: &ArrayView<f64, Ix2>) -> Array<f64, Ix2> {
    let num_quats = quat_wxyz.shape()[0];
    let mut yaws_rad = Array::<f64, Ix2>::zeros((num_quats, 1));
...
/// Convert a scalar-first quaternion to yaw.
/// In the Argoverse 2 coordinate system, this is counter-clockwise rotation about the +z axis.
pub fn _quat_to_yaw(quat_wxyz: &ArrayView<f64, Ix1>) -> f64 {
...
/// In the Argoverse 2 coordinate system, this is counter-clockwise rotation about the +z axis.
/// Parallelized for batch processing.
pub fn yaw_to_quat(yaw_rad: &ArrayView<f64, Ix2>) -> Array<f64, Ix2> {
    let num_yaws = yaw_rad.shape()[0];
    let mut quat_wxyz = Array::<f64, Ix2>::zeros((num_yaws, 4));
...
pub fn _yaw_to_quat(yaw_rad: f64) -> Array<f64, Ix1> {
    let qw = f64::cos(0.5 * yaw_rad);
    let qz = f64::sin(0.5 * yaw_rad);
    Array::<f64, Ix1>::from_vec(vec![qw, 0.0, 0.0, qz])
}

/// Reflect orientation across the x-axis.
/// (N,4) `quat_wxyz` orientation of `N` rigid objects.
pub fn reflect_orientation_x(quat_wxyz: &ArrayView<f64, Ix2>) -> Array<f64, Ix2> {
...
/// (N,4) `quat_wxyz` orientation of `N` rigid objects.
pub fn reflect_orientation_y(quat_wxyz: &ArrayView<f64, Ix2>) -> Array<f64, Ix2> {
    let yaw_rad = quat_to_yaw(quat_wxyz);
...
pub fn reflect_translation_x(xyz_m: &ArrayView<f64, Ix2>) -> Array<f64, Ix2> {
...
pub fn reflect_translation_y(xyz_m: &ArrayView<f64, Ix2>) -> Array<f64, Ix2> {
    let mut augmented_xyz_m = xyz_m.to_owned();
    augmented_xyz_m
        .slice_mut(s![.., 0])
...
pub fn sample_random_quat_wxyz() -> Array<f64, Ix1> {
    let distribution = StandardNormal;
    let qw: f64 = distribution.sample(&mut rand::thread_rng());
    let qx: f64 = distribution.sample(&mut rand::thread_rng());
    let qy: f64 = distribution.sample(&mut rand::thread_rng());
    let qz: f64 = distribution.sample(&mut rand::thread_rng());

...
class Sim2:
    ...
    def __init__(self, R: NDArrayFloat, t: NDArrayFloat, s: Union[int, float]) -> None:
        ...
        Raises:
            ValueError: If scaling factor is not a float or int.
            ZeroDivisionError: If `s` is close to zero.
        """
        assert_np_array_shape(R, (2, 2))
        assert_np_array_shape(t, (2,))

        if not (isinstance(s, float) or isinstance(s, int)):
            raise ValueError("Scaling factor is of invalid type, must be float or int.")
        if np.isclose(s, 0.0):
            raise ZeroDivisionError("3x3 matrix formation would require division by zero")

        self.R_: NDArrayFloat = R.astype(np.float32)
        self.t_: NDArrayFloat = t.astype(np.float32)
        self.s_ = float(s)
    ...
    def __repr__(self) -> str:
        ...
        trans = np.round(self.t_, 2)  # type: ignore
        return f"Angle (deg.): {self.theta_deg:.1f}, Trans.: {trans}, Scale: {self.s_:.1f}"
    ...
    def compose(self, S: Sim2) -> Sim2:
        ...
        return Sim2(
            R=self.R_ @ S.R_,
            t=self.R_ @ S.t_ + ((1.0 / S.s_) * self.t_),  # type: ignore
            s=self.s_ * S.s_
        )
        # fmt: on
    ...
    def inverse(self) -> Sim2:
        """Return the inverse."""
        Rt = self.R_.T
        sRt: NDArrayFloat = -Rt @ (self.s_ * self.t_)
    ...
    def transform_from(self, points_xy: NDArrayFloat) -> NDArrayFloat:
        ...
        transformed_point_cloud: NDArrayFloat = (points_xy @ self.R_.T) + self.t_

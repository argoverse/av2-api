...
def crop_xyz(
    points_xyz: Union[NDArrayFloat, NDArrayInt],
    ...
    """Crop Cartesian coordinates to a lower and upper boundary.
    ...
    Returns:
        (...,3) Tuple of cropped Cartesian coordinates and the (...,) corresponding boolean mask.
    """
    # Gather dimensions.
    n_dim = points_xyz.shape[-1]
    ...
    # Lower bound mask.
    lb_mask = np.greater_equal(points_xyz, lower_bound_inclusive)

    # Upper bound mask.
    ub_mask = np.less(points_xyz, upper_bound_exclusive)

    # Bound mask.
    b_mask = np.logical_and(lb_mask, ub_mask).all(axis=-1)
    return points_xyz[b_mask], b_mask

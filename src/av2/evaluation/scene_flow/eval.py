...
def angle_error(pred: NDArrayFloat, gt: NDArrayFloat) -> NDArrayFloat:
    ...
    eps = 1e-7
    dot_product = np.clip(np.sum(unit_label * unit_pred, axis=-1),
                          a_min=-1 + eps, a_max=1 - eps)
...
def metrics(pred_flow: NDArrayFloat, pred_dynamic: NDArrayBool,
            gt: NDArrayFloat, classes: NDArrayInt,
            dynamic: NDArrayBool, close: NDArrayBool, valid: NDArrayBool,
            object_classes: Dict[str, List[int]]) -> List[List[Union[str, float, int]]]:
    results = []

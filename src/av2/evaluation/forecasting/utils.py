...
def agent_velocity(agent: Dict[str, Any]) -> NDArrayFloat:
    ...
    Returns:
        ...
        List of velocities corresponding to each movement prediction_m for predicted agents
    """
...
def trajectory_type(agent: Dict[str, Any], class_velocity: Dict[str, float]) -> Union[str, List[str]]:
    ...
    if "future_translation_m" in agent:  # ground_truth
        ...
        linear_target = agent["current_translation_m"][:2] + time * agent["velocity"][:2]
        ...
        threshold = 1 + constants.FORECAST_SCALAR[len(agent["prediction_m"])] * class_velocity.get(agent["name"], 0)
        for i in range(agent["prediction_m"].shape[0]):
            static_target = agent["current_translation_m"][:2]
            linear_target = agent["current_translation_m"][:2] + time * agent["velocity"][i][:2]

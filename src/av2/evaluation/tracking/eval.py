...
def calculate_matched_scores(labels: Dict, predictions: Dict, sim_func: Callable) -> Tuple[np.ndarray, int]:
    scores = []
    n_gt = 0
    num_tp = 0
...
def recall_to_scores(scores: np.ndarray, recall_threshold: np.ndarray, n_gt: int) -> np.ndarray:
    # Sort scores.
    scores.sort()
    scores = scores[::-1]
...
if __name__ == "__main__":
    ...
    score_thresholds, tuned_metric_values, mean_metric_values = tune_score_thresholds(
        labels,
        track_predictions,
        objective_metric,
        classes,
